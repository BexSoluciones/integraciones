    ...
    public function uploadOrder($order, $cia, $closing, $connection_id)
    {
        ...
        try{
            if (count($order) > 0) {
                ...
                $chain .= str_pad($order[0]->nitcliente, 15, " ", STR_PAD_RIGHT); //Tercero cliente a facturar
                $chain .= str_pad($order[0]->succliente, 3, "0", STR_PAD_LEFT); //Sucursal cliente a facturar
                $chain .= str_pad($order[0]->nitcliente, 15, " ", STR_PAD_RIGHT); //Tercero cliente a despachar
                $chain .= str_pad($order[0]->succliente, 3, "0", STR_PAD_LEFT); //Sucursal cliente a despachar
                ...
                if($order[0]->fechorentregacli >= $order[0]->fecmov){
                    $chain.=substr($order[0]->fechorentregacli,0,4).substr($order[0]->fechorentregacli,5,2).substr($order[0]->fechorentregacli,8,2);
                }else{
                ...
                if($order[0]->ordendecompra != ''){
                    $chain .= str_pad($order[0]->ordendecompra, 15, "Y", STR_PAD_LEFT); //Orden de compra del Documento
                }else{
                ...
                $chain .= str_pad($order[0]->codfpagovta, 3, " ", STR_PAD_LEFT); //Condicion de pago
                $chain .= '0'; //Estado de impresión del documento
                $chain .= str_pad(utf8_decode($order[0]->mensajemov), 2000, " ", STR_PAD_RIGHT); //Observaciones del documento
                $chain .= str_pad('', 15, " ", STR_PAD_LEFT); //cliente de contado
                $chain .= '000'; //Punto de envio
                $chain .= str_pad($order[0]->cedula, 15, " ", STR_PAD_RIGHT); //Vendedor del pedido --nit vendedor 
                ...
                foreach ($order as $orderDetail) {
                    ...
                    if($orderDetail->prepack =='S'){
                        ...
                        $chain .= '02'; //Version del tipo de registro
                        $chain .= str_pad($config['IdCia'],3,"0",STR_PAD_LEFT); //compañia
                        $chain .= $order[0]->co; //Centro de operacion
                        $chain .= $order[0]->tipodoc; //Tipo de documento
                        $chain .= str_pad($order[0]->numvisita, 8, "0", STR_PAD_LEFT); //Consecutivo de documento
                        ...
                        if($order[0]->fechorentregacli >= $order[0]->fecmov){
                            $chain.=substr($order[0]->fechorentregacli,0,4).substr($order[0]->fechorentregacli,5,2).substr($order[0]->fechorentregacli,8,2); //Fecha de entrega del pedido
                        }else{
                            $chain.=substr($order[0]->fecmov,0,4).substr($order[0]->fecmov,5,2).substr($order[0]->fecmov,8,2); //Fecha de entrega del pedido
                        } 
                        $chain .= '000'; //Nro. dias de entrega del documento
                        $chain .= str_pad($order[0]->codprecio, 3, " ", STR_PAD_RIGHT); //Lista de precio-->{FIJARSE QUE HAY QUE TRAERLA DE LA TABLA MOVENC}
                        $chain .= str_pad(substr(trim($order[0]->nomunidademp),0,4),4," ",STR_PAD_RIGHT); //Unidad de medida-->pendiente
                ...
                Storage::disk('public')->put('export/bex_0010/bexmovil/pedidos_txt/' . $namefile, $chain);// CAMBIAR EL NOMBRE DE LA COMPAÑIA
